...
trait KinesisMatchers extends Matchers with CirceSupport {
  ...
  lazy val kinesis: AmazonKinesis = AmazonKinesisAsyncClientBuilder
    .standard()
    .withCredentials(container.dummyAWSCreds)
    .withEndpointConfiguration(
      new AwsClientBuilder.EndpointConfiguration(s"http://localhost:$portKinesis/", "us-east-1")
    )
    .build()
  ...
  ) = Matcher { kinesis: AmazonKinesis =>
    val actualEvents = KinesisUtil(kinesis).getRecordsJSON[T](streamName)

    MatchResult(
      actualEvents.size == expectedEvents.size && actualEvents
        .map(_._2)
        .toSet == expectedEvents.toSet,
