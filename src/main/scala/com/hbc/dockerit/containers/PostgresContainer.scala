...
trait PostgresContainer extends DockerKit with ScalaFutures with PostgresMatchers {
  ...
  private[this] def getPostgresPort(portsMapping: Map[Int, Int]) =
    portsMapping.getOrElse(
      AdvertisedPort,
      Assertions.fail(
        s"Couldn't find mapping for port $AdvertisedPort. (Found: ${container.getPorts()})"
      )
    )

  private[this] def buildUrl(host: String, port: Int): String =
    s"jdbc:postgresql://$host:$port/?loggerLevel=OFF"
